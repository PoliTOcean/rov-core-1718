...
def callback(data):
    ...
    path = '/home/vittorio/catkin_ws/src/politocean/scripts/Validation_results.txt'
    fileimu=open(path,'a')
    fileimu.write(str(data.data[0])+' '+str(data.data[1])+' '+str(data.data[2])+' '+str(data.data[3])+' '+str(data.data[4])+' '+str(data.data[5])+' '+str(angles[0])+' '+str(angles[1])+'\n')
    fileimu.close() 
